...
import {FILTER_BY_HIGH_SPEED, FILTER_BY_MIN_SPEED ,FILTER_BY_MIN_HP, FILTER_BY_HIGH_HP , ORDER_DEFAULT ,GET_POKEMON_PER_NAME ,SET_POKEMONS_TYPE_RENDER, GET_POKEMONS_TYPE , ADD_POKEMONS, SET_ORDER_A_Z, SET_ORDER_Z_A, FILTER_BY_ATTACK_MIN_MAX,FILTER_BY_ATTACK_MAX_MIN, SHOW_DB_POKEMONS, FILTER_BY_MIN_WEIGHT, FILTER_BY_HIGH_WEIGHT, FILTER_BY_MIN_DEFENSE, FILTER_BY_HIGH_DEFENSE, ADD_POKEMONS_API } from '../Redux/Types'
...
export const addPokemons = ()=> {
    ...
    export const addApiPokemons = ()=> {
      const myUrl = 'http://localhost:3001/pokemonsapi'
      return async (dispatch)=> {
      try {
          const { data } = await axios.get(myUrl)
          if(!data) return ('No hay pokemones en estos momentos')
  ...
  export const addPokemonsDb= ()=> {
    const myUrl = 'http://localhost:3001/pokemonsdb'
    return async (dispatch)=> {
    try {
        const { data } = await axios.get(myUrl)
        if(!data) return ('No hay pokemones en estos momentos')
       ...
       console.log('Error obteniendo los pokemones creados')
    }
    }
    }
...
export const fetchPokemonsTypes = (type) => 
  async(dispatch) => {
  try {
